...
int caller_save_registers[128] = {0};

int num_callee_save_registers = 0;
int callee_save_registers[128] = {0};
...
dw_loc_descr_ref
based_loc_descr (unsigned reg, int offset, struct dwarf_data *ddata)
{
  register dw_loc_descr_ref loc_result;
  register unsigned fp_reg = SP_REGNUM;
  if (reg == fp_reg)
    {
...
{
  register dw_die_ref ptr_die = lookup_type_die (type, ddata);
  if (ptr_die)
    return;
...
BOOL codegen (InterCode code, SymTab stab, const char *main_input_filename, FILE *asm_out_file, BOOL debug_info) 
{
    ...
    if  (!CodeGeneratorArm32 (code, stab, main_input_filename, asm_out_file, ddata))
        goto fail;
