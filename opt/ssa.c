...
{
    int sym_index;
    struct avl_table *tailvex, *headvex;
}IfNode; /* 顶点结点 */
typedef struct
{
...
{ /* 初始条件: 有向图G存在,v和有向图G中顶点有相同特征 */
  /* 操作结果: 在有向图G中增添新顶点v(不增添与顶点相关的弧,留待InsertArc()去做) */
    ...
    p = (struct pair *) avl_find (G->map, &v);
    if  (!p)
    {
        ...
        p->x = v; p->y = (*G). vexnum;
        avl_insert (G->map, p);
        (*G). xlist[(*G). vexnum].headvex = avl_create ((avl_comparison_func *) compare_integers, NULL, NULL);
        (*G). xlist[(*G). vexnum].tailvex = avl_create ((avl_comparison_func *) compare_integers, NULL, NULL);
        (*G). xlist[(*G). vexnum].sym_index = v;
        (*G). vexnum++;
    }
    return p->y;
}
...
static BOOL
is_connected (PhaseIFG *G,int i,int j)
{
    return avl_find (G->xlist[i].headvex, &j) || avl_find (G->xlist[j].headvex, &i);
}
...
{ /* 初始条件: 有向图G存在,v和w是G中两个顶点 */
    ...
    *f = i;
    free (avl_replace ((*G). xlist[j].headvex, f));
    f = (int *) xmalloc (sizeof (int));
    *f = j;
    free (avl_replace ((*G). xlist[i].tailvex, f));
    (*G). arcnum++; /* 弧数加1 */
}
 ...
 */
static void interfere_with_live(PhaseIFG *G, varpool_node vnode, bitmap liveout)
{
    bitmap_iterator bi;
    unsigned i;

    for (bmp_iter_set_init (&bi, liveout, 0, &i);
         bmp_iter_set (&bi, &i);
         bmp_iter_next (&bi, &i))
    {
        if  (i != vnode->uid)
        {
            InsertArc (G, InsertVex (G, i), InsertVex (G, vnode->uid));
            InsertArc (G, InsertVex (G, vnode->uid), InsertVex (G, i));
        }
    }
}

static void
calc_liveness (control_flow_graph cfun, varpool_node_set set)
{
    ...
    {
        bitmap_clear ((*block)->live_in);
        bitmap_clear ((*block)->live_out);
    }
    ...
    do
      {
           ...
           )
        {
            ...
            bitmap_copy ((*block)->live_out, work);

            /* 在反向指令遍历中删除定值，包括使用。  */
            for(  insn=(IRInst *)List_Last((*block)->insns)
               ;  insn!=NULL
               ...
               )
            {
                ...
                {
                    for(  ei2=(edge *)List_First((*ei)->dest->preds), i = 1
                       ;  ei2!=NULL
                       ;  ei2 = (edge *)List_Next((void *)ei2), i++
                       )
                        if  ((*ei2)->src == *block)
                            break;
                    for(  insn2=(IRInst *)List_Last((*ei2)->dest->insns)
                       ;  insn2!=NULL
                       ;  insn2 = (IRInst *)List_Prev((void *)insn2)
                       )
                    {
                        if  ((*insn2)->opcode == IRINST_OP_phi)
                        {
                            vnode = varpool_get_node (set, IRInstGetOperand (*insn2, i));
            ...
            if (bitmap_ior_into ((*block)->live_in, work))
                changed = TRUE;
        }
      }
    while (changed);
...
static void
buildIFG (control_flow_graph F, varpool_node_set set, PhaseIFG *G)
{
    basic_block* block;
    IRInst *instr, *curs;
    bitmap liveout;
       ...
       )
    {
        ...
        for(  instr=(IRInst *) List_Last((*block)->insns)
           ;  instr!=NULL
           ;  instr = (IRInst *) List_Prev((void *)instr)
           )
        {
            for (k = 0; k < IRInstGetNumOperands (*instr); k++)
                if  (IRInstIsOutput (*instr, k))
                    bitmap_set_bit (liveout, varpool_get_node (set, IRInstGetOperand (*instr, k))->uid);
            for (k = 0; k < IRInstGetNumOperands (*instr); k++)
            {
                if  (IRInstIsOutput (*instr, k))
                {
                    vnode = varpool_get_node (set, IRInstGetOperand (*instr, k));
                    interfere_with_live (G, vnode, liveout);
                }
            }
            tmp.first = tmp.current = 0;
            bitmap_copy (&tmp, liveout);
            for (k = 0; k < IRInstGetNumOperands (*instr); k++)
                if  (IRInstIsOutput (*instr, k))
                    bitmap_clear_bit (&tmp, varpool_get_node (set, IRInstGetOperand (*instr, k))->uid);
            if  ((*instr)->opcode != IRINST_OP_phi)
            {
                for (k = 0; k < IRInstGetNumOperands (*instr); k++)
                    if  (!IRInstIsOutput (*instr, k))
                        bitmap_set_bit (&tmp, varpool_get_node (set, IRInstGetOperand (*instr, k))->uid);
            }
            if  (! List_Next ((void *)instr))
            {
                for(  ei=(edge *) List_First((*block)->succs)
                   ;  ei!=NULL
                   ;  ei = (edge *) List_Next((void *)ei)
                ...
                {
                    for(  ei2=List_First((*ei)->dest->preds), k = 1
                       ;  ei2!=NULL 
                       ;  ei2 = List_Next((void *)ei2), k++
                       )
                        if  ((*ei2)->src == *block)
                            break;
                    for(  curs=(IRInst *) List_First((*ei)->dest->insns) 
                       ;  curs!=NULL
                       ;  curs = (IRInst *) List_Next((void *)curs)
                       )
                        if  ((*curs)->opcode == IRINST_OP_phi)
                            bitmap_set_bit (&tmp, varpool_get_node (set, IRInstGetOperand (*curs, k))->uid);
                }
            }
            bitmap_copy (liveout, &tmp);
            bitmap_clear (&tmp);
...
{ /* 初始条件: 有向图G存在 */
   /* 操作结果: 销毁有向图G */
    ...
    {
        avl_destroy ((*G).xlist[j].headvex, (avl_item_func *) free);
        avl_destroy ((*G).xlist[j].tailvex, (avl_item_func *) free);
    }
    free (G->xlist);
...
static void Display(PhaseIFG *G, varpool_node_set set, const char *file)
{ /* 输出有向图G */
    int i;
    int *p;
    FILE *fp;
...
/*  printf("共%d个顶点,%d条弧:\n",G->vexnum,G->arcnum);*/
    ...
    for(i=0;i<G->vexnum;i++)
    {
        ...
        fprintf (fp, "%s<SUB>%d</SUB>", stGetSymName (varpool_node_set_find (set, GetVex (G, i)->sym_index)->var), varpool_node_set_find(set, GetVex (G, i)->sym_index)->version);
        fprintf (fp, " </td></tr>");
        fprintf (fp, "</table>> ];\n");

/*      printf("顶点%s_%d: 入度: ",stGetSymName (varpool_node_set_find (set, GetVex (G, i)->sym_index)->var), varpool_node_set_find(set, GetVex (G, i)->sym_index)->version); */
        for(  p = (int *)avl_t_first (&trav, G->xlist[i].headvex)
           ;  p != NULL
           ;  p = (int *)avl_t_next (&trav)
           )
        {
            fprintf(fp, "   n%03d -- n%03d ;\n", i, *p);
/*          printf("%s_%d ",stGetSymName (varpool_node_set_find (set, GetVex (G, *p)->sym_index)->var), varpool_node_set_find(set, GetVex (G, *p)->sym_index)->version); */
        }
/*      printf("\n"); */
    }
    fputs ("}\n", fp);
...
rewrite_out_of_ssa (InterCode code, SymTab stab)
{
       ...
       )
    {
        ...
        for(  temp1 = (varpool_node)avl_t_first (&trav, set->nodes)
           ;  temp1 != NULL
           ;  temp1 = (varpool_node)avl_t_next (&trav)
           )
        {
            temp1->param = xmalloc (sizeof (struct variable_info));
            memset (temp1->param, '\0', sizeof (struct variable_info));
            ((struct variable_info *) temp1->param)->unresolvedNeighborMap = BITMAP_XMALLOC ();
            ((struct variable_info *) temp1->param)->phiCongruenceClass = BITMAP_XMALLOC ();
            bitmap_set_bit (((struct variable_info *) temp1->param)->phiCongruenceClass, temp1->uid);
        }
           ...
           )
        {
               ...
               )
            {
                if  ((*instr)->opcode == IRINST_OP_phi)
                {
                    candidateResourceSet = BITMAP_XMALLOC ();
                    for (i = 0; i < IRInstGetNumOperands (*instr); i++)
                    {
                        temp1 = varpool_get_node (set, IRInstGetOperand (*instr, i));
                        bitmap_clear (((struct variable_info *) temp1->param)->unresolvedNeighborMap);
                    }
                    BITMAP_XFREE (candidateResourceSet);
                }
            }
        }
#endif
...
# endif
        ...
        {
            BITMAP_XFREE (((struct variable_info *) temp1->param)->phiCongruenceClass);
            BITMAP_XFREE (((struct variable_info *) temp1->param)->unresolvedNeighborMap);
            free (temp1->param);
        }
